...
use function Proxy\Helper\getPackageLength;
use function Proxy\Helper\getString;
use function Proxy\Helper\initConfig;
use Proxy\Helper\ProcessHelper;
use Proxy\Log\Log;
use Proxy\MysqlPacket\AuthPacket;
use Proxy\MysqlPacket\BinaryPacket;
use Proxy\MysqlPacket\MySqlPacketDecoder;
use Proxy\MysqlPacket\MySQLPacket;
use Proxy\MysqlPacket\OkPacket;
use Proxy\MysqlPacket\ErrorCode;
use Proxy\MysqlPacket\RandomUtil;
use Proxy\MysqlPool\MySQLException;
use Proxy\MysqlPool\MySQLPool;
use Proxy\Parser\ServerParse;
use Proxy\Route\RouteService;
...
class ProxyServer extends BaseServer
{
    ...
    public function onReceive(\swoole_server $server, int $fd, int $reactor_id, string $data)
    {
        self::go(function () use ($server, $fd, $reactor_id, $data) {
            if (!isset($this->source[$fd]->auth)) {
                throw new ProxyException('Must be connected before sending data!');
            }
            ...
            self::go(function () use ($server, $fd, $reactor_id, $data) {
                ...
                // 处理Proxy 命令
                if ($bin->data[4] == ProxyPacket::$PROXY) {
    ...
    public function onWorkerStart(\swoole_server $server, int $worker_id)
    {
        self::go(function () use ($server, $worker_id) {
            if ($worker_id >= CONFIG['server']['swoole']['worker_num']) {
                ProcessHelper::setProcessTitle('Proxy task    process');
            } else {
                ProcessHelper::setProcessTitle('Proxy worker-' . $worker_id . '  process');
                try {
        ...
        foreach ($this->dbConfig as $key => $value) {
            ...
            while ($value['startConns']) {
                ...
                switch (explode(DB_DELIMITER, $key)[0]) {
                    case 'read':
                        $mysql->query('/*Proxy test sql*/select sleep(0.01)');
                        break;
                    case 'write':
                        $mysql->query('/*Proxy test sql*/set autocommit=1');
                        break;
                }
                $clients[] = $mysql;
                $value['startConns']--;
    ...
    private function existsDBKey(\swoole_server $server, int $fd, string $model, string $key)
    {
        //如果没有写库则抛出异常
        if (!isset($this->dbConfig[$key])) {
            $message = 'Proxy@Database config ' . ($this->source[$fd]->database ?: '') . ' ' . $model .
                ' is not exists!';
