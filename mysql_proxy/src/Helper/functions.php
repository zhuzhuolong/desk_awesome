...
use Proxy\Command\ServerCommand;
 ...
 * @return array
 */
function getBytes(string $data): array
{
    $bytes = [];
    $count = strlen($data);
    for ($i = 0; $i < $count; ++$i) {
 ...
 * @return array
 */
function array_copy(array $array, int $start = 0, int $len = 0)
{
    if (!$len) $len = count($array);
 ...
 * @return array
 */
function getMysqlPackSize(int $size, int $length = 3): array
{
    $sizeData[] = $size & 0xff;
 ...
 * @return int
 */
function shr16(int $x, int $bits): int
{
 ...
 */
function initConfig(string $dir): array
{
    $config = [];
    ...
    if (isset($config['server']['mode'])) {
        replace_constant($config['server']['mode'], (string)SWOOLE_PROCESS);
    } else {
        $config['server']['mode'] = SWOOLE_PROCESS;
    }

    if (isset($config['server']['sock_type'])) {
        replace_constant($config['server']['sock_type'], (string)SWOOLE_SOCK_TCP);
    } else {
        $config['server']['sock_type'] = SWOOLE_SOCK_TCP;
    }

    if (isset($config['server']['swoole_client_sock_setting']['sock_type'])) {
        replace_constant($config['server']['swoole_client_sock_setting']['sock_type'], (string)SWOOLE_SOCK_TCP);
    } else {
    ...
    if (isset($config['server']['logs']['config']['system']['log_path'])) {
        ...
        throw new ProxyException('ERROR:server.logs.config.system.log_path 配置项不存在!');
    }
    if (isset($config['server']['logs']['config']['mysql']['log_path'])) {
        mk_log_dir($config['server']['logs']['config']['mysql']['log_path']);
    } else {
        throw new ProxyException('ERROR:server.logs.config.mysql.log_path 配置项不存在!');
    }
    if (isset($config['server']['swoole']['log_file'])) {
        mk_log_dir($config['server']['swoole']['log_file']);
    } else {
        throw new ProxyException('ERROR:server.swoole.log_file 配置项不存在!');
    }
    if (isset($config['server']['swoole']['pid_file'])) {
        mk_log_dir($config['server']['swoole']['pid_file']);
    } else {
        throw new ProxyException('ERROR:server.swoole.pid_file 配置项不存在!');
    }
 ...
 * @return string
 */
function absorb_version_from_git(): string
{
    $defaultVersion = ServerCommand::PROXY_VERSION;
    $hasGit = ProcessHelper::run('type git >/dev/null 2>&1 || { echo >&2 "false" ;}', ROOT)[2];
    if ($hasGit !== "false") {
        $tagInfo = ProcessHelper::run('git describe --tags HEAD', ROOT)[1];
 ...
 * @param int $exitCode
 */
function proxy_error($message, $exitCode = 0)
{
 ...
 */
function array_iconv($data, string $output = 'utf-8'): array|string|null
{
