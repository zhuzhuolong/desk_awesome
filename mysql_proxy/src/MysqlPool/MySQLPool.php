...
class MySQLPool extends Base
{
    ...
    //配置项
    protected static mixed $connsConfig;
    //所有的连接
    ...
    //等待连接对应的协程通道
    protected static array $yieldChannel  = [];
    //初始连接数
    protected static array $initConnCount = [];
    //连接最后交互时间
    protected static array $lastConnsTime = [];
    //MySql服务端
    protected static mixed $mysqlServer;
    ...
    public static function fetch(string $connName, \swoole_server $server, int $fd)
    {
        ...
        if ((count(self::$busyConns[$connName]) + count($connsPool) + self::$pendingFetchCount[$connName] +
            ...
            if (false === $client) {
                --self::$pendingFetchCount[$connName];
                $message = 'Proxy@Reach max connections! Cann\'t pending fetch!';
    ...
    public static function initConn(\swoole_server $server, int $fd, string $connName, $tryStep = 0)
    {
        ...
        if (false == $conn->connect($serverInfo['host'], $serverInfo['port'], $serverInfo['timeout'] ?? 0.1)) {
            --self::$initConnCount[$connName];
            $message = 'Proxy@MySQL server has gone away';
            $errMessage = self::writeErrMessage(1, $message, ErrorCode::ER_HAS_GONE_AWAY);
            if ($server->exist($fd)) {
                $server->send($fd, getString($errMessage));
            }
        ...
        if ($client === false) {
            ...
            if ($tryStep < 3) {
                ...
                $message = 'Proxy@Connection ' . $serverInfo['host'] . ':' . $serverInfo['port'] .
     ...
     * @param \swoole_server      $server
     * @param int                 $fd
     * @param \Proxy\MysqlProxy $conn
     * @param string              $connName
     *
     * @return mixed
     *
     * @throws \Proxy\MysqlPool\MySQLException
     * @throws \Proxy\ProxyException
     */
